    ...
    try:
        ...
        with mido.open_output(port_name) as out:
            ...
            if envelope:
                ...
                out.send(mido.Message('control_change', control=CC_MAPPINGS["Consciousness_Emergence"], value=attack_cc))
                out.send(mido.Message('control_change', control=CC_MAPPINGS["Thought_Persistence"], value=sustain_cc))
                out.send(mido.Message('control_change', control=CC_MAPPINGS["Mental_Release"], value=release_cc))
                out.send(mido.Message('control_change', control=CC_MAPPINGS["Filter_Emergence"], value=attack_cc))
                time.sleep(0.02)
            ...
            out.send(mido.Message('control_change', control=CC_MAPPINGS["Harmonic_Complexity"], value=harmonic_complexity))
            out.send(mido.Message('control_change', control=CC_MAPPINGS["Phase_Coherence"], value=phase_coherence))
            time.sleep(0.02)
            ...
            for i, osc in enumerate(oscillators):
                ...
                if role_cc_key in CC_MAPPINGS:
                    ...
                    out.send(mido.Message('control_change', control=role_cc, value=amplitude))
                
                # Send fundamental strength for first oscillator
                if i == 0:
                    out.send(mido.Message('control_change', control=CC_MAPPINGS["Fundamental_Strength"], value=amplitude))
                
                # Note on with amplitude as velocity
                out.send(mido.Message('note_on', note=pitch, velocity=amplitude))
            ...
            if interpretation:
                ...
                out.send(mido.Message('control_change', control=CC_MAPPINGS["Emotional_Clarity"], value=intensity_cc))
                time.sleep(0.02)
